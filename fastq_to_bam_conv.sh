...
mkfifo $FQ1.conv.fifo
mkfifo $FQ2.conv.fifo

zcat $FQ1 | $SCRIPT_DIR/convert_stream_illumina-sanger.pl > $FQ1.conv.fifo &
zcat $FQ2 | $SCRIPT_DIR/convert_stream_illumina-sanger.pl > $FQ2.conv.fifo &


# convert pair
java -Xmx2g -jar $PICARD_DIR/FastqToSam.jar TMP_DIR=$PICARD_TEMP FASTQ=$FQ1.conv.fifo FASTQ2=$FQ2.conv.fifo OUTPUT=$FINAL_BAM \
