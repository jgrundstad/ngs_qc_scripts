...
sub submit_jobs {
	...
	if(query_qstat($job_name)) {
		my $jj = query_qstat($job_name);
		print "Unique check: $jj\n";
    ...
    while($#cmds > -1) {
        ...
        if(query_qstat($job_name) >= $max_jobs) {
            ...
            for(1 .. $max_jobs - query_qstat($job_name)) {
				...
				my $cmds_left = $#cmds + 1;
				my @cmd_date = split / /,`date`;
				my $cmd_date_slice = join " ", @cmd_date[0..4];
				print STDERR "[$cmd_date_slice] commands left in batch[$batch]: $cmds_left/$total_cmds    in queue: $q/$max_jobs(max)\n";
				last if ($cmds_left == 0);
            }
        }
    }
